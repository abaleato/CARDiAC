...
class experiment:
    def __init__(self, sigma, z_mean, sigma_zshift, sigma_zwidth, nside, bvec, z_min_int=0.005, z_max_int = z_mean+2,
        ...
        self.z_array = np.zeros((npix, n_samples_of_chi))
        ...
        chimean_shifts_array = np.random.normal(loc=modulation_of_mean_of_draws, scale=self.sigma_chishift, size=npix)
        width_shifts_array = np.random.normal(loc=modulation_of_mean_of_draws, scale=self.sigma_chiwidth, size=npix)
        ...
        phi_perturbed_array = (Planck18.H(self.z_array[0, :]) / c).value * dndz_perturbed
        phi_fid_array = (Planck18.H(self.z_array[0, :]) / c).value * dndz_fid

        # Normalize the selection function so that \int_{0}^{inf} dr phi(r) = 1
        phi_norm, error = quadrature(interp1d(self.chi_array, phi_fid_array), self.chi_min_int, self.chi_max_int,
                                     tol=1e-20, miniter=3000)
        phi_perturbed_array *= phi_norm ** (-1)
        phi_fid_array *= phi_norm ** (-1)
        dndz_fid_normed = dndz_fid / phi_norm
        phi_fid = interp1d(self.chi_array, phi_fid_array)
        ...
        for i in range(len(self.chi_array)):
            delta_p_maps_upsampled[:, i] = hp.ud_grade(delta_p_maps[:, i], nside_upsampling)
        ...
        for i in range(delta_p_lm_of_chi.shape[0]):
            ...
            Cl_deltap_of_chi1_chi2[l, :, :] += factor * np.outer(delta_p_lm_of_chi[i, :],
                                                                 np.conj(delta_p_lm_of_chi[i, :])).real / (2 * l + 1) / \
                                               pixwinf[l] ** 2

        # We'll need the interpolated version of this
        self.cldp_interp = interp1d(self.chi_array, np.diagonal(Cl_deltap_of_chi1_chi2, axis1=1, axis2=2), axis=-1)
        ...
        def plot_realizations(self):
            ...
            for pixel_id in np.random.randint(0, npix, 5):
                plt.plot(self.z_array[0, :], phi_perturbed_array[pixel_id, :], label='Actual, pixel {}'.format(pixel_id))

            # Plot the fiducial dndz
            plt.plot(self.z_array[0, :], phi_fid_array, color='k', lw=3, label='fiducial')
        ...
        def plot_fluctuations(self):
            ...
            plt.plot(self.z_array[pix_idx, :], delta_p_maps[pix_idx, :])
        ...
        def plot_ClDphi_of_chi(self):
            ...
            for l_to_plot in np.linspace(10, lmax, 8, dtype=int):
                plt.plot(self.chi_array, np.diagonal(Cl_deltap_of_chi1_chi2, axis1=1, axis2=2)[l_to_plot, :],
...
def mode_coupling_bias(exp, ells, lprime_max=100):
            ...
            for L in np.arange(np.abs(l - lprime), np.abs(l + lprime) + 1, 1):
                if (l + lprime + L) % 2 == 0:
                    ...
                    integ, error = quadrature(integrand_conv_term, exp.chi_min_int, exp.chi_max_int,
                                              args=(lprime, Pkgg_interp_1D, prefactor, exp.cldp_interp), miniter=1000, maxiter=2000,
                                              tol=1e-12)
                    conv_bias[i] += integ
    return conv_bias

def additive_bias(exp, ells):
    ...
    for i, l in enumerate(ells):
        print('Working on l={}; iteration {}/{}'.format(l, i+1, len(ells)))
        Clchi1chi2_interp = interpolate.RegularGridInterpolator((exp.chi_array, exp.chi_array), exp.Cl_deltap_of_chi1_chi2[l, :, :],
                                       method='linear', bounds_error=True, fill_value=0)

        additive_bias[i], error = quadrature(integrand_additive_term, exp.chi_min_int,
                                             exp.chi_max_int, args=(Clchi1chi2_interp, exp.chi_min_int, exp.chi_max_int),
                                             miniter=3, maxiter=5, tol=1e-20)
    return additive_bias

def unbiased_term(exp, ells):
    ...
    for i, l in enumerate(ells):
        ...
        X, Y = np.meshgrid((l + 0.5) / exp.chi_array, exp.chi_array, indexing='ij')
        Pkgg_interp_1D = interp1d(exp.chi_array, np.diagonal(exp.Pkgg_interp((X, Y))))
        clgg_unbiased[i], error = quadrature(integrand_unbiased_auto_term, exp.chi_min_int, exp.chi_max_int,
